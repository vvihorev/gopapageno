...
func (p *Parser) Parse(ctx context.Context, src []byte) (*Token, error) {
	...
	stateStackPools := make([]*Pool[stack[CyclicAutomataState]], p.concurrency)
	statePools := make([]*Pool[CyclicAutomataState], p.concurrency)
	...
	for thread := 0; thread < p.concurrency; thread++ {
		...
		stateStackPools[thread] = NewPool[stack[CyclicAutomataState]](int(stackPtrPoolBaseSize), WithConstructor[stack[CyclicAutomataState]](newStack[CyclicAutomataState]))
		statePools[thread] = NewPool[CyclicAutomataState](int(stackPtrPoolBaseSize), WithConstructor[CyclicAutomataState](NewCyclicAutomataStateBuilder(p.MaxPrefixLength)))
	...
	for thread := 0; thread < p.concurrency; thread++ {
		...
		if p.ParsingStrategy != COPP {
			go workers[thread].parse(ctx, NewParserStack(ptrPools[thread]), tokensLists[thread], nextToken, false, resultCh, errCh)
		} else {
			go workers[thread].parseCyclic(ctx, NewCyclicParserStack(ptrPools[thread], stateStackPools[thread], statePools[thread]), tokensLists[thread], nextToken, false, resultCh, errCh)
	...
	if p.concurrency > 1 {
		if p.reductionStrategy == ReductionSweep {
			...
			if p.ParsingStrategy != COPP {
				go workers[0].parse(ctx, NewParserStack(stackPtrPoolFinalPass), finalPassInput, nil, true, resultCh, errCh)
			} else {
				// TODO: Init Stack
				go workers[0].parseCyclic(ctx, nil, finalPassInput, nil, true, resultCh, errCh)
			}
			...
			for p.concurrency--; p.concurrency >= 1; p.concurrency-- {
				for i := 0; i < p.concurrency; i++ {
					...
					if p.ParsingStrategy != COPP {
						go workers[i].parse(ctx, stack.(*ParserStack), input, nil, true, resultCh, errCh)
					} else {
						go workers[i].parseCyclic(ctx, stack.(*CyclicParserStack), input, nil, true, resultCh, errCh)
					}
				}

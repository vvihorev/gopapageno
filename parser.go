...
type parserPools struct {
	...
	states      []*Pool[CyclicAutomataState]
	...
	sweepStates     *Pool[CyclicAutomataState]
}
...
func (p *Parser) Parse(ctx context.Context, src []byte) (*Token, error) {
	...
	for thread := 0; thread < p.concurrency; thread++ {
		...
		if p.ParsingStrategy != COPP {
			...
			s = NewCyclicParserStack(p.pools.stacks[thread], p.pools.stateStacks[thread], p.pools.states[thread])
		}
	...
	if p.concurrency > 1 {
		if p.reductionStrategy == ReductionSweep {
			...
			if p.ParsingStrategy != COPP {
				...
				s = NewCyclicParserStack(p.pools.sweepStack, p.pools.sweepStateStack, p.pools.sweepStates)
			}
...
func (p *Parser) init(src []byte) {
	...
	if p.ParsingStrategy == COPP {
		...
		p.pools.states = make([]*Pool[CyclicAutomataState], p.concurrency)
	}
